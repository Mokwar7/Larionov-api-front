...
function Header({ isLogged, cart, getCart, cleanCart, removeCountFromCart, addCountToCart, removeFromCart }) {
  ...
  function handleAddCountCart(e) {
    ...
    let size = e.target.closest('.cart__count-cont').querySelectorAll('.cart__btn')[1].getAttribute('size')
    addCountToCart(art, size, e)
  }
      ...
      <div className="header__container">
        <div className="header__cont">
          ...
          <nav className={"header__nav header__nav_menu" + (clickedBurger == true ? ' header__nav_active' : '')}>
            <NavLink to="/catalog" className="header__link hover7" onClick={handleCloseBurger}>Каталог</NavLink>
            <NavLink to="/contacts" className="header__link hover7" onClick={handleCloseBurger}>Контакты</NavLink>
            <NavLink to="/delivery_payment" className="header__link hover7" onClick={handleCloseBurger}>Доставка и оплата</NavLink>
          </nav>
        </div>
        ...
        <div className={"cart" + (isCart == true ? ' cart_active' : '')}>
          ...
          {
            cart.length != 0 ?
              <a href="https://t.me/rawkom" className="card__buy">Заказать</a> : false
          }
        </div>
      </div>
      <div className={"cart__back" + (isCart == true ? ' cart__back_active' : '')} onClick={handleCloseCart}></div>
      <div className={"cart__back" + (clickedBurger == true ? ' cart__back_active' : '')} onClick={handleClickBurger}></div>
    </header>
  )
}
